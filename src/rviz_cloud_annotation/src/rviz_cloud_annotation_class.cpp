...
RVizCloudAnnotation::RVizCloudAnnotation(ros::NodeHandle &nh1) : m_nh(nh1)
{
  ...
  m_nh.param<std::string>(DATASET_FOLDER, param_stringA, DATASET_FOLDER_DEFAULT); // 加载点云数据集路径

  m_dataset_folder = param_stringA;

  m_nh.param<std::string>(IMAGE_FOLDER, param_stringA1, IMAGE_FOLDER_DEFAULT); // 加载图片数据集路径


  m_image_folder = param_stringA1;

  m_nh.param<std::string>(ANNOTATION_CLOUD_FOLDER, param_stringB, ANNOTATION_CLOUD_FOLDER_DEFAULT); // 加载点云输出路径

  m_annotation_cloud_folder = param_stringB;

  m_nh.param<std::string>(ANNOTATION_FILE_FOLDER, param_stringC, ANNOTATION_FILE_FOLDER_DEFAULT); // 加载标注记录路径

  m_annotation_file_folder = param_stringC;

  m_nh.param<std::string>(BBOX_NAME_FOLDER, param_stringD, BBOX_NAME_FOLDER_DEFAULT); // 加载BBOX输出路径

  m_bbox_folder = param_stringD;

  m_nh.param<std::string>(LABEL_NAME_FOLDER, param_stringE, LABEL_NAME_FOLDER_DEFAULT); // 加载标签输出路径

  m_label_folder = param_stringE;

  m_nh.param<std::string>(LINE_NAME_FOLDER, param_stringF, LINE_NAME_FOLDER_DEFAULT); // 加载Line输出路径

  m_line_folder = param_stringF;

  // modified on 2020.01.14 by feng
  image_transport::ImageTransport imageTrans(m_nh);
  std::string param_string_image;
  m_nh.param<std::string>(PARAM_IMAGE_LABEL_TOPIC, param_string_image, PARAM_DEFAULT_IMAGE_LABEL_TOPIC);
  // // std::cout<<"param_string:$$$$$$$$$$$$$$$$$$$$$$$$$$$$"<<param_string_image.c_str()<<std::endl;
  m_image_label_pub = imageTrans.advertise(param_string_image, 1);
  // m_control_image_show= m_nh.subscribe(param_string_image, 1, &RVizCloudAnnotation::showImageLabel, this);

  m_nh.param<std::string>(LOG_FILE, param_stringO, LOG_FILE_DEFAULT); // 加载标注日志路径
  ...
  //加载所有待标注数据集
  LoadDataSet(param_stringA, param_stringA1, param_stringB, param_stringC, param_stringD, param_stringE, param_stringF);
  //初始化一帧点云，并开启标记
  InitNewCloud(*nh);
...
//加载数据集及路径信息
// void RVizCloudAnnotation::showImageLabel(const sensor_msgs::ImageConstPtr& msg)
// {
//   // modified on 2020.01.14 by feng
//   // std::cout<<"$$$$$$$$$$$$$$$$$$$$$$$$$$$$"<<param_string_image.c_str()<<std::endl;
//   try
//   {
//     std::cout<<"start showing image$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$"<<std::endl;
...
void RVizCloudAnnotation::LoadDataSet(std::string A, std::string A1, std::string B, std::string C, std::string D,
                                      std::string E, std::string F)
{
  ...
  for (int k = 0; k < m_pcd_files.size(); k++)
  {
    ...
    if (ifNew)
    {
      ...
      // modified on 2020.01.014 by feng
      // name = A1 + index + std::string(".png");
      name = A1 + index + std::string(".jpg");
...
void RVizCloudAnnotation::InitNewCloud(ros::NodeHandle &nh)
{
  ...
  // modified on 2020.01.14 by feng
  // m_nh.param<std::string>(PARAM_IMAGE_LABEL_TOPIC_2, param_string,
  //                         PARAM_DEFAULT_IMAGE_LABEL_TOPIC_2);
  // m_image_show = m_nh.subscribe(param_string, 1, &RVizCloudAnnotation::showImageLabel, this);
  ...
  m_on_auto_plane_sub = m_nh.subscribe(param_string, 1, &RVizCloudAnnotation::onAutoPlane, this);
...
{
  std::string pcd_type;
  m_nh.param<std::string>(PARAM_NAME_PCD_TYPE, pcd_type, PARAM_DEFAULT_PCD_TYPE);
  ...
  // code for plane annotation, should be removed if you don't need plane annotation for decreasing running time.
...
void RVizCloudAnnotation::onControlYawChange(const std_msgs::Int32 &msg)
{
  ...
  if (abs(BBOX_YAW_ANGLE) == 90 || abs(BBOX_YAW_ANGLE) == 0 || abs(BBOX_YAW_ANGLE) == 180)
  {
    if_tilt = false;
  }
  else
...
//生成地面 并发布　Marker
void RVizCloudAnnotation::generatePlane(const Uint64Vector &ids, bool editMode)
{
  if (editMode == true) // Add
  {
    const Uint64Vector changed_labels = m_undo_redo.SetControlPointVector(ids, 0, 26);
    SendControlPointsMarker(changed_labels, true);
    SendPointCounts(changed_labels);
    SendUndoRedoState();
...
void RVizCloudAnnotation::generateBbox(const PointXYZRGBNormalCloud &cloud, bool if_tilt)
{
  ...
  if (if_tilt == false)
  {
    ...
    AddBbox(0, 0, shape[0], shape[1], shape[2], shape[3], shape[4], shape[5], if_tilt);
  }
  else
  {
                                  ...
                                  bool tilt)
{
  ...
  ROS_INFO("BBOX[%d]:%f %f %f %f %f %f %f %f %f %f", BBOX_ID, BBOX_SET[BBOX_ID][0], BBOX_SET[BBOX_ID][1],
           BBOX_SET[BBOX_ID][2], BBOX_SET[BBOX_ID][3], BBOX_SET[BBOX_ID][4], BBOX_SET[BBOX_ID][5], BBOX_SET[BBOX_ID][6],
           BBOX_SET[BBOX_ID][7], BBOX_SET[BBOX_ID][8], BBOX_SET[BBOX_ID][9]);
}
...
void RVizCloudAnnotation::FinalLabel(PointXYZRGBNormalCloud &cloud)
{
  ...
  const Uint64Vector changed_labels = m_undo_redo.SetControlPointVector(kerb_ids, 0, 1);
  SendControlPointsMarker(changed_labels, true);
  SendPointCounts(changed_labels);
  SendUndoRedoState();
